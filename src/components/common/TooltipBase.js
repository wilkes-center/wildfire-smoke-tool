 ...
 */
const TooltipBase = ({ children, content, position = 'top', className = '' }) => {
  const [isVisible, setIsVisible] = useState(false);

  const getTooltipClasses = () => {
    const baseClasses =
      'absolute z-50 px-2 py-1 text-xs font-medium text-white bg-green/95 rounded shadow-lg whitespace-nowrap pointer-events-none transition-opacity duration-200 font-redhat';
  ...
  return (
    ...
    >
      {isVisible && <div className={getTooltipClasses()}>{content}</div>}
      {children}
    </div>
  );
};
