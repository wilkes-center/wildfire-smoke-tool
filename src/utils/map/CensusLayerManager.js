...
class CensusLayerManager {
  ...
  notifyProgress(stage, progress) {
    this.progressCallbacks.forEach(callback => callback({ stage, progress }));
  }
    ...
    this.initPromise = (async () => {
      try {
        ...
        map.addLayer({
          ...
          maxzoom: 12,
          paint: {
            'fill-color': isDarkMode
              ? CENSUS_COLORS['fill-color'].dark
              : CENSUS_COLORS['fill-color'].light,
            'fill-opacity': 0,
            'fill-outline-color': isDarkMode
              ? CENSUS_COLORS['fill-outline-color'].dark
              : CENSUS_COLORS['fill-outline-color'].light
          }
        });
